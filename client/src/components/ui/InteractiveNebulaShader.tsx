 ...
 * Adapted for chat-area only: container is absolute inset-0
 * relative to its parent (ChatPanel), not fixed viewport.
 */
export function InteractiveNebulaShader({
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      (uniforms.iMouse.value as THREE.Vector2).set(e.clientX, window.innerHeight - e.clientY);
    };

    window.addEventListener("resize", onResize);
    window.addEventListener("mousemove", onMouseMove);

    // Also observe container resize for side-panel changes
    const resizeObserver = new ResizeObserver(onResize);
    resizeObserver.observe(container);
    onResize();

    renderer.setAnimationLoop(() => {
      (uniforms.iTime.value as number) = clock.getElapsedTime() as unknown as number;
      // Type-safe assignment
      (material.uniforms.iTime.value as number) = clock.getElapsedTime();
